...
ensure_coreutils_installed() {
    dnf list rust-coreutils | MATCH installed
    while read p; do
        util_path="$(which $p)"
...
ensure_coreutils_absent() {
    dnf list rust-coreutils | NOMATCH installed
...
ensure_findutils_installed() {
    dnf list rust-findutils | MATCH installed
...
ensure_findutils_absent() {
    dnf list rust-findutils | NOMATCH installed
...
ensure_diffutils_installed() {
    if [[ "$(lsb_release -rs)" != "24.04" ]]; then
        dnf list rust-diffutils | MATCH installed
...
ensure_diffutils_absent() {
    dnf list rust-diffutils | NOMATCH installed
